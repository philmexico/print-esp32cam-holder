...
module clamp_positive(first_half = true) {
    union() {
        ...
        if (first_half == true) {
            ...
            // lock
            translate([- (bar_radius + clamp_thickness + clamp_thickness), -.5 * clamp_thickness, clamp_height / 2 - clamp_thickness])rotate([0, 45, 0])cube([2 * clamp_thickness, .5 * clamp_thickness, 2 * clamp_thickness]);
        } else {
            // hinge
            translate([hinge_position_x, 0, (clamp_height - hinge_height) / 2 + hinge_height / 4])cylinder(h = hinge_height / 2, r = hinge_radius);
            // lock
            translate([- (bar_radius + clamp_thickness + clamp_thickness), 0 * clamp_thickness, clamp_height / 2 - clamp_thickness])cube([2 * clamp_thickness, .5 * clamp_thickness, 2 * clamp_thickness]);
        }
    }
}
...
module clamp_negative(first_half = true) {
    ...
    if (first_half == true) {
        // hinge
        translate([hinge_position_x, 0, (clamp_height - hinge_height) / 2 + hinge_height / 4 - printer_tolerance])cylinder(h = hinge_height / 2 + printer_tolerance * 2, r = hinge_radius);
        // lock
//        translate([-1 * (bar_radius + clamp_thickness + clamp_thickness / 2), 0, clamp_height / 2])rotate([90, 0, 0])cylinder(h = clamp_thickness / 2, r = screwhole_radius);
    } else {
        // hinge
        ...
        // lock
//        translate([-1 * (bar_radius + clamp_thickness + clamp_thickness / 2), clamp_thickness / 2, clamp_height / 2])rotate([90, 0, 0])cylinder(h = clamp_thickness / 2, r = screwhole_radius);
    }
    translate([-1 * (bar_radius + clamp_thickness + clamp_thickness / 2), clamp_thickness / 2, clamp_height / 2])rotate([90, 0, 0])cylinder(h = clamp_thickness, r = screwhole_radius);
}
